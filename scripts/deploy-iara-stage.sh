#!/bin/bash
#
# Deploy IAra to Stage Environment (pptnc-stage)
#
# Usage:
#   ./scripts/deploy-iara-stage.sh           # Full deploy
#   ./scripts/deploy-iara-stage.sh --dry-run # Show commands without executing
#   ./scripts/deploy-iara-stage.sh --skip-tests # Skip test execution
#
...
REGION="us-east1"
SERVICE_NAME="iara-stage"
FIRESTORE_DATABASE_ID="pptnc-stage"
...
for arg in "$@"; do
  case $arg in
    ...
    --skip-tests)
      ...
      ;;
    --help|-h)
      echo "Deploy IAra to Stage Environment"
      echo ""
...
log_step "Configuring Docker for Artifact Registry"
...
run_cmd pnpm --filter iara build
log_success "Next.js build completed"

# Step 3: Build Docker image (context = monorepo root)
log_step "Building Docker image"
run_cmd docker build -f apps/iara/Dockerfile -t ${SERVICE_NAME}:${IMAGE_TAG} .
