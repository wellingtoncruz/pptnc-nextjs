...
async function analyzeSubset() {
    ...
    const REPORT_READY = "linkedin_enrichment_subset_ready.md";
    const REPORT_SKIPPED = "linkedin_enrichment_subset_skipped.md";

    console.log(`Analyzing subset of ${SKIP_IDS.length} episodes...`);

    let readyReport = `# LinkedIn Enrichment - Subset Ready\n\n`;
    let skippedReport = `# LinkedIn Enrichment - Subset Skipped\n\n`;
    let readyCount = 0;
    let skippedCount = 0;
    ...
    for (const id of SKIP_IDS) {
        ...
        if (hasManualLinkedin) {
            readyCount++;
            readyReport += `## ${title} (${id})\n`;
            readyReport += `> **Manual Update Detected**\n`;
            readyReport += `| Guest | Role | Company | LinkedIn | Method |\n`;
            readyReport += `| :--- | :--- | :--- | :--- | :--- |\n`;

            guests.forEach(g => {
                if (g.linkedin) {
                    readyReport += `| ${g.name} | ${g.role || "-"} | ${g.company || "-"} | ${g.linkedin} | Manual |\n`;
                }
            });
            readyReport += `\n`;
            continue; // Skip enrichment logic if manually set
        }

        // If no manual update, try to enrich again (in case description changed)
        // 1. Detect Co-hosts
        ...
        if (hasMatches) {
            ...
            processGuests.forEach((guest, index) => {
                ...
                if (match) {
                    readyReport += `| ${guest.name} | ${guest.role || "-"} | ${guest.company || "-"} | ${match.url} | ${match.method} |\n`;
                }
            });
            readyReport += `\n`;
        } else {
            skippedCount++;
            skippedReport += `## ${title} (${id})\n`;
            if (uniqueUrls.length > 0) skippedReport += `> URLs found: ${uniqueUrls.join(', ')}\n`;
            else skippedReport += `> No URLs found.\n`;
            skippedReport += `\n`;
        }
