...
/**
 * Link schema - a reference link attached to an episode (Epic 26).
 *
 * Producers register links + descriptions in the Links phase of the wizard
 * (episodes only). Links flagged `includeInDescription` are appended
 * deterministically to the YouTube description at publish time via
 * buildCompleteYouTubeDescription (5th section, after chapters).
 *
 * @example
 * {
 *   url: "https://github.com/exemplo/repo",
 *   description: "Repositório do projeto discutido",
 *   includeInDescription: true
 * }
 */
export const LinkSchema = z.object({
  url: z.string().url('URL inválida'),
  description: z.string().min(1, 'Descrição do link é obrigatória'),
  includeInDescription: z.boolean().default(false),
})
...
export const VideoSchema = z.object({
  ...
  links: z.array(LinkSchema).optional(), // Links de referência do episódio (Epic 26 — fase Links)
