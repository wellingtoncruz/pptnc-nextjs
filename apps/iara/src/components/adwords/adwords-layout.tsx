...
export function AdwordsLayout() {
  ...
  // Server-side typeFilter: 'episode' handles filtering — no client-side filter needed
  ...
  const handleVideoSelect = useCallback((videoId: string) => {
    router.push(`${pathname}?view=adwords&selected=${videoId}`)
  }, [router, pathname])

  return (
    <div data-testid="adwords-layout" className="flex h-full">
      {/* Video sidebar */}
      ...
      <div className="flex-1 overflow-hidden">
        {selectedVideoId && selectedVideo ? (
          <div data-testid="adwords-work-area" className="h-full">
            <AdwordsWorkPanel videoId={selectedVideoId} video={selectedVideo} />
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center gap-3 h-full text-muted-foreground">
            ...
            <p className="text-sm">Selecione um episódio para gerar o guia de tráfego pago</p>
          </div>
        )}
      </div>
    </div>
