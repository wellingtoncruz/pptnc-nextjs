...
describe('useVideos', () => {
  ...
  it('carrega vídeos com sucesso', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/videos?page=1&limit=20')
  })
  ...
  it('atualiza página quando setPage é chamado', async () => {
    ...
    expect(mockFetch).toHaveBeenLastCalledWith('/api/videos?page=2&limit=20')
  })
  ...
  it('filtra por tipo quando setTypeFilter é chamado', async () => {
    ...
    expect(mockFetch).toHaveBeenLastCalledWith('/api/videos?page=1&limit=20&type=episode')
  })
